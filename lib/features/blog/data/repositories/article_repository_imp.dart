...
class ArticleRepositoryImpl implements ArticleRepository {
  ...
  Future<Either<Failure, List<Article>>> getAllBookmarks() async {
    try {
      final articles = await localDataSource.getAllBookmarks();
  ...
  @override
  Future<Either<Failure, Article>> deleteArticle(String articleId) async {
    try {
      final responseData = await remoteDataSource.deleteArticle(articleId);
      if (responseData != null) {
        final article = Article.fromJson(responseData);
        log('Article created $article');
        return Right(article);
      ...
      return const Left(ServerFailure('Error Deleting Blog'));
    }
  }
