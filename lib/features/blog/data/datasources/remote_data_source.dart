...
class RemoteDataSource implements BlogRemoteDataSource {
  ...
  @override
  Future<dynamic> deleteArticle(String articleId) async {
    String endpoint = 'article/$articleId';
    log("fetching: $baseUrl/$endpoint");
    final sharedPreferences = sl<SharedPreferences>();
    final token = sharedPreferences.getString('auth_token');
    ...
    try {
      ...
      final responseData = json.decode(response.body);
      log("response (delete_datalayer): $responseData");
      if (response.statusCode == 200 && responseData['success'] == true) {
